...
public interface ReportDataRepository extends JpaRepository<ReportData, ReportDataId> {
	...
	Collection<ReportData> findByTypeIdAndLpuIdAndDateTimeBetweenOrderByDateTime(Long reportTypeId, Integer lpuId, LocalDateTime dtFrom,
			LocalDateTime dtTo);

	Collection<ReportData> findByTypeIdAndDateTimeBetweenOrderByDateTime(Long reportTypeId, LocalDateTime parse, LocalDateTime parse2);

	Collection<ReportData> findByTypeIdAndLpuIdAndDateTimeAfterOrderByDateTime(Long reportTypeId, Integer lpuId, LocalDateTime parse);

	Collection<ReportData> findByTypeIdAndDateTimeAfterOrderByDateTime(Long reportTypeId, LocalDateTime parse);

	Collection<ReportData> findByTypeIdAndLpuIdAndDateTimeBeforeOrderByDateTime(Long reportTypeId, Integer lpuId, LocalDateTime parse);
